...
- 라우트로 사용된 컴포넌트에 `match` `location`과 함께 전달되는 **props**
    - `match` → *현재 컴포넌트가 어떤 경로 규칙에 의해 보이는지에 대한 정보*
    - `location` → *웹 애플리케이션의 현재 주소에 대한 정보*
- history 객체 통해 **컴포넌트 내 구현하는 메서드에서 라우터 API 호출** 가능
    - ex) 특정 버튼을 눌렀을 때 뒤로 가거나 로그인 후 화면 전환, 다른 페이지로 이탈 방지 시 활용
...
- `withRouter`를 사용할때는 **컴포넌트를 내보내 줄 때 함수**로 감쌈
...
- 라우트로 사용된 컴포넌트가 아니어도 `match` `location` `history` 객체에 접근 가능
    ...
    - `withRouter`를 사용하면 현재 자신을 보여주고 있는 라우트 컴포넌트를 기준으로 `match` 가 전달된다.

## Swtich 컴포넌트

- **여러 Route를 감싸서 그중 일치하는 단 하나의 라우트만을 렌더링** 시켜준다.
- 사용하면 모든 규칙과 일치하지 않을 때 보여 줄 *Not Found* 페이지 구현 가능
(**규칙에 적용되지 않는 페이지를 색출 가능**)
...
- Link와 비슷!
- **현재 경로와 Link에서 사용하는 경로가 일치**하는 경우 **특정 스타일 혹은 CSS 적용**
    - NavLink에서 **링크 활성화 시** 스타일 적용 : `activeStyle 값 props`
    - NavLink에서 **CSS 클래스**를 적용 : `activeClassName 값 props`
